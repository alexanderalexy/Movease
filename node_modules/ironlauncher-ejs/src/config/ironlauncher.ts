...
import { ICLIConfig, IronlauncherConfig, IronLauncherTemplate, IronLauncherVariant } from '../types'
...
class IronLauncher implements IronlauncherConfig {
  ...
  #setDryRun(startingValue = false) {
    ...
    this.#dryRun =
      this.#isBoolean(startingValue) || this.#isBoolean(dryRun) || this.#isBoolean(dryRunCommand)
  }

  #setBase(startingValue = false) {
    const { base, b } = this.flags
    this.#base = this.#isBoolean(startingValue) || this.#isBoolean(base) || this.#isBoolean(b)
  }
  ...
  #setVerbose(startingValue = false) {
    ...
    this.#verbose = this.#isBoolean(startingValue) || this.#isBoolean(v) || this.#isBoolean(verbose)
  }

  #setAuth(startingValue = false) {
    const { a = false, auth = false } = this.flags
    this.#auth = this.#isBoolean(startingValue) || this.#isBoolean(a) || this.#isBoolean(auth)
  }

  #isBoolean(option: string | boolean = false) {
    return (
      (typeof option === 'boolean' || JSON.parse(JSON.stringify(option)) === 'boolean') &&
      option == true
    )
  }
  ...
  public debug(): void {
    ...
    console.log(`---- VARIANT ----`)
    console.log(`VARIANT: ${this.#auth ? 'auth' : this.#base ? 'base' : 'not_defined'}`)
